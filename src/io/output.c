...
void write_ascii_array(int nn, int perLine, float *array, FILE *fp)
{
    ...
    switch(perLine) {
        ...
        case 6:
            ...
            break;
        default:
            fprintf(stdout,"write_ascii_array with %d perLine is not impl\n",perLine);
            exit(0);
            break;
    }
}
...
#ifdef ZLIB_DEBUG_MODE
    fprintf(stdout,"zlibcompress:o(%4d)/c(%4d)/%4d/%f%%\n",nn,*nn2,nn - *nn2,(float)(nn - *nn2)/nn);
#endif
...
void vtm_file_header(FILE * fp)
{

}

void vtm_file_trailer(FILE * fp)
{
